...
<section class=" overflow-auto bg-white flex sm:flex-row flex-col bg-clip-border gap-2 rounded-[.95rem]  m-5  text-slate-600">
    <section class="w-full h-full border-r-2 <?php echo isset($_SESSION['log_user_type']) && $_SESSION['log_user_type'] == 'student' ? 'sm:w-[60%]' : '' ?> p-5">
        ...
        <div class="card-body flex flex-col items-center p-3 h-[70vh] sm:p-5 overflow-hidden hover:overflow-auto scroll-smooth gap-5" id="actSched">
            <div class="flex min-w-[40rem] <?php echo  (isset($_SESSION['log_user_type']) && $_SESSION['log_user_type'] == 'student' ? 'w-[40rem]' : 'w-[50rem]') ?> shadow rounded transition duration-500 transform hover:scale-110 hover:bg-slate-300 cursor-pointer justify-start items-center">
